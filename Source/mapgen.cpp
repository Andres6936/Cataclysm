...
bool Item_area::load_data(std::istream& data, std::string name)
{
	...
	while (data >> item_ident)
	{
		...
		if (item_ident.substr(0, 2) == "w:")
		{ // It's a weight, e.g. a chance
			tmp_chance.chance = atoi(item_ident.substr(2).c_str());

		}
		else if (item_ident.substr(0, 2) == "c:")
		{ // It's a count - for all_items
		...
		{ // Otherwise, it should be a item name
...
bool Item_amount_area::load_data(std::istream& data, std::string name)
{
	...
	while (data >> ident)
	{
		...
		{ // Otherwise, it should be a group name
...
void Tile_substitution::load_data(std::istream& data, std::string name)
{
	...
	while (data >> ident)
	{
		...
		{ // Should be a char or list of chars
			characters += ident;
		}
	}
...
// If we're a relational map, rotate based on neighbors...
			...
			world_ter[0]->has_flag(WTF_RELATIONAL))
		{
			...
			{ // Faster to check who DOESN'T have it
				if (!neighbor[DIR_NORTH])
				{
					rotate(DIR_EAST);
				}
...
std::string mapgen_flag_name(Mapgen_flag flag)
{
	...
	case MAPFLAG_NULL:
		return "NULL";
	case MAPFLAG_AUTOSTAIRS:
		return "autostairs";
	case MAPFLAG_NOROTATE:
		return "norotate";
	case MAPFLAG_SEPARATE_FURNITURE:
		return "separate_furniture";
	case MAPFLAG_MAX:
		return "ERROR - MAPFLAG_MAX";
	default:
		return "UNKNOWN MAPGEN_FLAG";
	}
