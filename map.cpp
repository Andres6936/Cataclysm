...
{
	if (!is_smashable())
	{  // This verifies that terrain != NULL
		return "";
	}
...
// Next, destroy the furniture
	...
	if (!is_smashable())
	{  // This also verifies that terrain != NULL
		return "";
	}
...
// Stat bonuses
			 ...
			 it++)
		{
			...
			{
			case STAT_STRENGTH:
				stat = user->stats.strength;
				break;
			case STAT_DEXTERITY:
				stat = user->stats.dexterity;
				break;
			case STAT_INTELLIGENCE:
				stat = user->stats.intelligence;
				break;
			case STAT_PERCEPTION:
				stat = user->stats.perception;
				break;
			}
